...
- **Auth**: the whole site sits behind Django's `LoginRequiredMiddleware` (added in `base.py`); mark genuinely public views with `@login_not_required`. First-time setup is an **onboarding wizard** (`core.views`, URLs under `/onboarding/`, names `core:onboarding-*`). Step 1 (`/onboarding/account/`, `core:onboarding-account`) creates the single admin account **immediately** (needed for the logged-in steps) and is only reachable while no user exists. Steps Tax → Workplace → Contract → Terms are **deferred**: each step's raw POST is held in `request.session["onboarding"]` (a payload that already passed `is_valid()`, so re-binding it on revisit re-shows the input with no errors → back-navigation keeps its place) and everything is written to the DB together, atomically, only on the Terms step's **Finish** (`_commit_onboarding`). The wizard reuses the real `WorkplaceForm`/`WorkplaceContractForm`/`ContractTermSetForm` and their templates via an `onboarding=True` context flag (transient unsaved Workplace/Contract objects supply display names). `OnboardingRequiredMiddleware` funnels anonymous fresh installs to the account step and logged-in-but-unfinished users to `/onboarding/`; completion signal = a `TaxProfile` **and** a `ContractTermSet` exist (cached in `session["onboarding_complete"]`, also exposed to templates by `core.context_processors.onboarding_status` so `base.html` hides the nav until done). Account username **is** the email (`OnboardingUserCreationForm`, validated + copied to `User.email`); password similarity is reworded via `core.validators.EmailSimilarityValidator`. View tests that hit normal pages must mark onboarding complete (see `LoggedInTestCase`).
